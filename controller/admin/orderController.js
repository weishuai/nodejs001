...
const softDeleteOrder = async (req, res) => {
  try {
    ...
    let result = await dbService.update(Order, query,updateBody);
    if (!result){
      return res.recordNotFound();
    }
...
const deleteManyOrder = async (req, res) => {
  try {
    ...
    let deletedOrder = await dbService.destroy(Order,query);
    return res.success({ data :{ count :deletedOrder.length } });
  }
  catch (error){
    return res.internalServerError({ message:error.message });  
  }
...
const softDeleteManyOrder = async (req, res) => {
  try {
    ...
    const options = {};
    let updatedOrder = await dbService.update(Order,query,updateBody, options);
    if (!updatedOrder) {
      return res.recordNotFound();
    }
    return  res.success({ data :{ count: updatedOrder.length } });
